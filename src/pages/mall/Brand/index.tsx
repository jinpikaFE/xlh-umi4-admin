...
import {
  ...
  editBrand,
  queryBrandList,
} from '@/services/mall/brand/BrandController';
...
const AttrVal: React.FC = () => {
  ...
  const edit = async (item: any) => {
    setCItem({
      ...
      logo: [
        {
          uid: item?.logo,
          name: item?.logo?.split('/')?.[item?.logo?.split('/')?.length - 1],
          // name: '富文本',
          status: 'done',
          url: item?.logo,
          file_link: item?.logo,
  ...
  const columns: ProColumns<NBrand.BrandEntity>[] = [
    {
      title: '序号',
      dataIndex: 'id',
      hideInSearch: true,
  ...
  const onFinish = async (values: any) => {
    const relVal = {
      ...values,
      logo: values?.logo?.[0]?.file_link,
    };
    if (cItem) {
      // 编辑逻辑，后端要操作组件数据和角色数据
      const res = await editBrand({ ...relVal, id: cItem?.id });
      if (res?.code === 200) {
      ...
      // 新增逻辑，后端要操作组件数据和角色数据
      const res = await addBrand(relVal);
      if (res?.code === 200) {
        message.success(res?.message || '创建成功');
        setVisibleDrawer(false);
        refTable?.current?.reload();
